...
class Fotophire
{
    ...
    private function makeImage(array $fileInfo, int $destW, int $destH, string $savePath): array
    {
        ...
        if ($res['sc'] != 'ok') {
            return Reply::To('err', $res['msg'], $res['data']);
        }
        ...
        //如果是png或webp
        if ($imgTypeCode == 3 || $imgTypeCode == 18) {
            // 给画布上底色
    ...
    private function resourceHandler($imagePath)
    {
        try {
            ...
            $imageTypeMap = [
                ...
                18 => 'imagecreatefromwebp',  // 添加 WebP 支持
            ];
            ...
            if (!array_key_exists($fileInfo[2], $imageTypeMap)) {
                ...
                return Reply::To(
                    'err',
                    '图片格式支持gif\jpg\png\webp',
                    [
                        'fileInfo' => $fileInfo,
                        'fileInfo2' => $fileInfo[2]
                    ]
                );
            }
    ...
    private function save($file, string $savePath, int $imgTypeCode): array
    {
        try {
            ...
            switch ($imgTypeCode) {
                ...
                case 18:
                    // WebP 文件保存
                    if (!imagewebp($file, $savePath)) {
                        return Reply::To('err', 'WebP 图像保存失败');
                    }
                    break;
                default:
