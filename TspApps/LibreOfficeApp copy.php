...
class LibreOfficeApp
{
    ...
    private $defaultFont = 'WenQuanYi Micro Hei';
    private $fontMsg = [];
    ...
    public function DocxToPdf(string $path, string $outputDir = null)
    {
        ...
        $pdfFileName = basename($localPath, '.docx') . '.pdf';
        ...
        try {
            ...
            if ($returnVar === 0) {
                if (file_exists($targetFilePath) && is_readable($targetFilePath)) {
                    ...
                    return Reply::To('err', 'PDF文件不存在或无法读取', [
                        'targetDir' => $targetDir,
